...
class VectorService:
    ...
    def connect_to_index(self):
        ...
        try:
            ...
            if self.index_name not in index_names:
                ...
                region = self._normalize_region(self.environment)
    ...
    @staticmethod
    def _normalize_region(environment: str) -> str:
        """
        Normalize Pinecone environment strings to a serverless region.
        Example: "us-east-1-aws" -> "us-east-1".
        """
        if not environment:
            return "us-east-1"
        if environment.endswith("-aws"):
            return environment[: -len("-aws")]
        return environment
