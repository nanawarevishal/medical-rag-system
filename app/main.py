...
from app.services.advanced_rag_pipeline_service import AdvancedRAGPipelineService
...
async def pipeline_query(payload: PipelineQueryRequest):
    """
    Pipeline query endpoint with boolean flags to enable/disable services.
    """
    ...
    if not advanced_rag_service:
        raise HTTPException(
            status_code=503,
            detail="Advanced RAG pipeline not initialized. Please configure API keys in .env file.",
        )
    ...
    try:
        result = await advanced_rag_service.answer_query(
            ...
            merge_top_k=payload.merge_top_k,
...
def initialize_services():
    ...
    global embedding_service, vector_service, rag_service, advanced_rag_service, sql_service, cache_service, query_cache_service
